...
const HomeScreen: React.FC<Props> = ({ navigation }: Props) => {
  ...
  return (
    <View style={[styles.container, navigation.getParam('chooseUsername') ? { backgroundColor: 'grey', opacity: 0.5 } : null]}>
      <ImageBackground source={image} style={styles.image}>
        <LinearGradient colors={['black']} style={styles.LinearGradient} />
        {navigation.getParam('chooseUsername') ? (
          <CreateUsername email={navigation.getParam('email')} id={navigation.getParam('id')} />
        ) : null}
